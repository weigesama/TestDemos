...
public class OptLockerTest {
    ...
    public void testUpdateByIdSucc() {
        ...
        userUpdate.setVersion(1); //这个版本号是条件，更新版本号是这个的行，并把版本号+1
    ...
    public void testUpdateByEntitySucc() {
        ...
        qw = new QueryWrapper<>();
        qw.eq("version", 1);
        Assert.assertEquals("No records found with version=1", 0, userMapper.selectCount(qw).intValue());
        qw = new QueryWrapper<>();
        qw.eq("version", 2);
        Assert.assertEquals("All records with version=1 should be updated to version=2", count, userMapper.selectCount(qw).intValue());
    }
